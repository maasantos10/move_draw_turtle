...
void turtleDraw(const int cmds[])
{
	...
	do {  // until END_OF_DATA is encountered
		...
		case TURN_LEFT: // Turn left
			...
			--direction; //change from NORTH/SOUTH to WEST
...
void moveTurtle( short floor[][NCOLS]
	           , const int numOfMoves
	           , const enum Dirs currDir
	           , const short penPos
	           , int* rowPtr, int* colPtr)
{

	int row = 0, column = 0, i; // local variable to store 2 dimension array
	int newNumOfMoves = numOfMoves;
	
	switch (currDir) {

		case NORTH: // UP
			
			if (numOfMoves > *rowPtr) {
				newNumOfMoves = *rowPtr;
			}
			
			switch(penPos) {
				case TRUE:
					for (i = 0; i < newNumOfMoves; i++)
					{
					...
					break;
				case FALSE:
					*rowPtr -= newNumOfMoves;
					break;
				default:
					printf("[MoveDrawTurtle] error North moveTurtle()");
					break;
			}
		...
		case EAST: // RIGHT
			if (numOfMoves > (NCOLS - *colPtr - 1)) {
				newNumOfMoves = (NCOLS - *colPtr - 1);
			}
			...
			case TRUE:
				...
				break;
			case FALSE:
				*colPtr += newNumOfMoves;
				break;
			default:
				puts("\n[MoveDrawTurtle] error East moveTurtle()\n");
				break;
			}
		...
		case SOUTH: // DOWN
			if (numOfMoves > (NROWS - *rowPtr - 1)) {
				newNumOfMoves = (NROWS - *rowPtr - 1);
			}
			...
			case FALSE:
				*rowPtr += newNumOfMoves;
				break;
			default:
				puts("\n[MoveDrawTurtle] error South moveTurtle()\n");
				break;
			}
		...
		case WEST: // LEFT
			if (numOfMoves > *colPtr) {
				newNumOfMoves = *colPtr;
			}
			...
			case FALSE:
				*colPtr -= newNumOfMoves;
				break;
			default:
				puts("\n[MoveDrawTurtle] error West moveTurtle()\n");
				break;
			}
		...
		default:
			puts("\n\n[MoveDrawTurtle] ERROR - invalid direction in moveTurtle().");
...
void displayFloor(const short floorSurface[][NCOLS])
{
	...
	for (row = 0; row < NROWS; ++row) { // ROW Iteration to control EAST/WEST or LEFT/RIGHT moviments
		for (column = 0; column < NCOLS; ++column) { // COLUMN Iteration to control NORTH/SOUTH or UP/DOWN moviments 
			floorSurface[row][column] == TRUE ? printf("*") : printf(" "); // ternary condition to print empty or *
		} // end for
		printf("\n");
	} // end for 
